...
nfc.on('reader', async reader => {
	...
	reader.on('card', async card => {
			...
			// var data = await reader.read(4, 872); // await reader.read(4, 16, 16); for Mifare Classic cards
			...
			// let exampleTagAsBuffer = utils.hexStringToByte(exampleTagAsHexString)
			// console.log('exampleTagAsBuffer', Buffer.from(exampleTagAsBuffer))
			// let tagAsBuffer = exampleTagAsBuffer;
			try {
				// var tagBlocks3and4 = await reader.read(3, 8); // only block 3 & 4

				// Header parsing
				var tagBufferBlocks0to4 = await reader.read(0, 20);
				// console.log(tagBufferBlocks0to4);

				let tagHeaderValues = nfcTag.parseHeader(tagBufferBlocks0to4);
				console.log(tagHeaderValues);

				// tag is readable and has a ndef message
				if(tagHeaderValues.hasTagReadPermissions && tagHeaderValues.isTagFormatedAsNdef && tagHeaderValues.hasTagANdefMessage) {
					...
					// ndef parsing
					let parsedRecords = nfcTag.parseNdef(tagBufferFromBlock4);
          console.log('PPP', parsedRecords)
