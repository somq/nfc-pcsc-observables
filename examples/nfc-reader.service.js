...
class NfcReaderService {
   ...
   */
  onCard$ = this.onReader$.switchMap(readerEvent => {
    // console.log(readerEvent);
    return Rx.Observable.fromEvent(readerEvent, 'card');
  });
...
nfcReader.onReaderEnd$.subscribe(readerEnd => {
  if (DEBUG) { pretty.info(`device removed`); }
});
...
// aCardHasBeenRead - when a card has been read and processed
nfcReader.aCardHasBeenRead$.subscribe(data => {
  console.log('A card has been read and processed', data)
});

// aCardHasBeenWritten - when a card has been written and processed
nfcReader.aCardHasBeenWritten$.subscribe(data => {
